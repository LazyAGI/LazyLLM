...
本文档描述 `lazyllm.tools.git.review` 模块的完整 PR review 端到端流程。入口为 `runner.py` 中的 `review()` 函数，依次经过预分析（架构解析、历史规范提取）、六轮 LLM 分析（R1 hunk 审查 → R2a PR 设计文档 → R2 架构师评审 → R3 Agent 验证 → RMod 修改必要性分析 → R4 合并去重）、测试覆盖检查（RCov）、静态 Lint 融合，最终将结果发布到 GitHub / GitLab / Gitee / GitCode。
...
Lint 分析在各 LLM 轮次之前独立运行，不消耗 LLM 调用预算。
