...
本文档描述 `lazyllm.tools.git.review` 模块的完整 PR review 端到端流程。入口为 `runner.py` 中的 `review()` 函数，依次经过预分析（架构解析、历史规范提取）、静态 Lint 分析、六轮 LLM 分析（R1 hunk 审查 → R2a PR 设计文档 → R2 架构师评审 → R3 Agent 验证 → RMod 修改必要性分析 → R4 合并去重）、测试覆盖检查（RCov），最终将结果发布到 GitHub / GitLab / Gitee / GitCode。
...
review(pr_number, ...)
  ...
  └─ _run_six_rounds (R1 → R2a → R2 → R3 → RMod → R4)
  └─ _run_coverage_check (RCov，并行，动态超时) → rcov_issues
  └─ 合并：final_comments + rcov_issues → all_comments
  └─ 清理 clone 子目录
...
| 14 | RCov | 测试覆盖检查：识别未覆盖符号，评估缺口 → `rcov_issues`（绕过 R4 去重直接追加） |
| 15 | 发布 | 提交平台 review；更新 checkpoint `UPLOAD` 阶段 |
| 16 | 清理 | 删除 `{pr_dir}/clone/`；保留 `checkpoint.json` |
...
**目标**：将 R1、R2、R3、RMod 的 issue 列表 + lint_issues 合并为最终的、无重复的评论列表。RCov issue 绕过本阶段，在 R4 之后直接追加。
...
RCov 结果保存在 `ReviewStage.RMOD` checkpoint key（`rcov_issues`）下。断点续传时直接加载缓存，跳过所有 LLM 调用。
...
```
CLONE → ARCH → SPEC → PR_SUMMARY → R1 → R2A → R2 → R3 → RMOD → RCOV → FINAL → UPLOAD
```
