...
class GitAgent:
    def __init__(self, project_path: str, llm: OnlineChatModule, language: str = 'zh'):
        ...
        self.docstring_manager = LLMDocstringManager(llm=llm, pattern='fill', skip_on_error=True, language=language)
        self.module_dict = {}  
    ...
    def _gen_module_dict(self):
        ...
        for root, dirs, files in os.walk(self.project_path):
            ...
            for file in files:
                if file.endswith('.py') and file not in {'__init__.py', 'setup.py', 'conftest.py', 'wsgi.py', 'asgi.py'}:
    ...
    def _process_module(self, module, f_module_name=""):
        m_dict = {module.__name__: {}}
        for name, obj in inspect.getmembers(module, inspect.isclass):
            if not getattr(obj, '__module__', "").startswith(module.__name__):
                continue
            m_dict[module.__name__][f"{f_module_name}.{name}"] = {'doc':obj.__doc__, 'obj': obj}
            m_dict[module.__name__][f"{f_module_name}.{name}"]['method'] = {}
            for method_name, method_obj in inspect.getmembers(obj, inspect.isfunction):
                m_dict[module.__name__][f"{f_module_name}.{name}"]['method'][f"{f_module_name}.{name}.{method_name}"] = {'doc':method_obj.__doc__, 'obj': method_obj}
        for name, obj in inspect.getmembers(module, inspect.isfunction):
            if not getattr(obj, '__module__', "").startswith(module.__name__):
                continue
            m_dict[module.__name__][f"{f_module_name}.{name}"] = {'doc':obj.__doc__, 'obj': obj}
        if not m_dict[module.__name__]:
            return {}
        return m_dict
    ...
    def _process_package(self, module, skip_modules):
        ...
        for importer, modname, ispkg in pkgutil.walk_packages(module.__path__, module.__name__ + "."):
            if any(modname.startswith(skip_mod) for skip_mod in skip_modules):
                continue
            if ispkg:
                m_dict[module.__name__]['children'] |= self._process_package(importer.find_module(modname).load_module(modname), 
                                                                             skip_modules)
            ...
            try:
                ...
                if any(modname.startswith(mod) for mod in processed_module):
                    continue
                m_dict[module.__name__]['module'] |= self._process_module(submodule, f_module_name=modname)
            except Exception as e:
                LOG.info(f"Skipping {modname} due to import error", e)
        if not m_dict[module.__name__]['children'] and not m_dict[module.__name__]['module']:
            return {}
        return m_dict
    ...
    def _register_tools(self):
        ...
        def get_module_doc(module_name: str) -> str:
            ...
            Args:
                module_name (str): Complete module name, from top-level to bottom-level, separated by dots (.), e.g. "a.b.c".
            Returns:
    ...
    def start_mkdocs_server(self, port=8333) -> None:
        ...
        if not os.path.exists(docs_dir_base) or not os.path.exists(os.path.join(docs_dir_base, "mkdocs.yml")):
            raise ValueError("Documentation directory or mkdocs.yml file does not exist, please generate automatically or manually first.")
        ...
        try:
            ...
            mkdocs_process = subprocess.Popen(
                ["mkdocs", "serve", "-f", os.path.join(docs_dir_base, "mkdocs.yml"), "-a", f"0.0.0.0:{port}"],
    ...
    def _generate_readme(self) -> None:
        ...
        readme_content = re.sub(r'^<think>.*?</think>\s*', '', readme_content, flags=re.MULTILINE|re.DOTALL)
                ...
                for module_name, module_info in info['module'].items():
                    ...
                    for obj_name, obj_info in module_info.items():
                        ...
                        if isinstance(obj_info, dict):
                            add_doc(module_name, obj_name, obj_info.get('doc', ''), '##')
                            if 'method' in obj_info:
                                for func_name, func_info in obj_info['method'].items():
                                    method_name = f"{obj_name}.{func_name.split('.')[-1]}"
                                    add_doc(module_name, method_name, func_info.get('doc', ''), '###')
    ...
    def _translate_docs(self, docs_dir_zh: str, docs_dir_en: str) -> None:
        ...
        for root, dirs, files in os.walk(docs_dir_zh):
            ...
            for file in files:
                ...
                LOG.info(f"Translated doc file: {os.path.relpath(zh_file_path, self.project_path)} -> {os.path.relpath(en_file_path, self.project_path)}")
