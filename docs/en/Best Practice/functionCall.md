...
`FunctionCall` returns the assistant message generated by the model. When a tool call is triggered, the message contains both `tool_calls` (the structured calls made by the model) and `tool_calls_results` (the output returned by each tool execution). These data are also synchronized into `lazyllm.locals['tool_calls']` and `lazyllm.locals['tool_call_results']` for use in the next round. If the model does not choose any tool, it directly returns a string. To complete the full reasoning loop automatically, use [FunctionCallAgent][lazyllm.tools.agent.FunctionCallAgent], as shown below:
...
### FunctionCallAgent Output Process
> - agent input
```text
What's the weather like today in Tokyo.
```

1. When an input arrives, it is passed directly to the [FunctionCall][lazyllm.tools.agent.FunctionCall] module.
> - function call output
```text
{'role': 'assistant', 'content': '', 'tool_calls': [{'id': 'xxx', 'type': 'function', 'function': {'name': 'get_current_weather', 'arguments': {'location': 'Tokyo, Japan', 'unit': 'celsius'}}}]}
```
> - Non–function call output
```text
今天的东京天气温度是10度 Celsius。
```

2. Check whether the result from [FunctionCall][lazyllm.tools.agent.FunctionCall] is a tool call, or whether the maximum number of iterations has been reached. If it is a tool call, return to Step 1. If it is not a tool call or the maximum number of iterations is reached, continue to the next step.

3. If the maximum number of iterations is reached, an exception is thrown. If the model generates a normal result, it is returned directly.
> - Throwing an exception after reaching the maximum number of iterations
```text
