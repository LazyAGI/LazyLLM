...
FROM_DATA_TO_LLM_ZH_INTRO = '''本课程旨在为开发者和算法工程师提供一套**从数据到模型再到应用**的 LLM 全栈实战指南。课程核心聚焦于**数据工程**，并将其深度融入到模型训练的每一个环节——从底座预训练 (Pre-training) 到指令微调 (SFT)，再到人类价值观对齐 (RLHF/GRPO)。
