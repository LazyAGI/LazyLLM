...
public:
    ...
    explicit TextSplitterBase(unsigned chunk_size = 1024, unsigned overlap = 200, unsigned worker_num = 0,
        const std::string& model_path = "")
        : NodeTransform(worker_num), _chunk_size(chunk_size), _overlap(overlap)
    {
        ...
        _tokenizer = std::make_shared<SentencePieceTokenizer>(model_path);
    }
...
protected:
    virtual std::vector<ChunkUnit> split_recursive(const std::string_view& view, int chunk_size) {
        int token_amount = get_token_amount(view);
        if (token_amount <= chunk_size) return {ChunkUnit{view, true, token_amount}};

        auto [text_splits, is_sentence] = _get_splits_by_fns(view);
        std::vector<ChunkUnit> results;
        for (const auto& segment : text_splits) {
            const int seg_token_size = get_token_amount(segment);
            if (seg_token_size <= chunk_size) {
                results.push_back(ChunkUnit{segment, is_sentence, seg_token_size});
            } else {
    ...
    virtual std::vector<std::string> _merge(std::vector<ChunkUnit> splits, int chunk_size) {
        if (splits.empty()) return {};
        if (splits.size() == 1) return {splits.front().text};

        ChunkUnit end_split = splits.back();
        if (end_split.token_size == chunk_size && _overlap > 0) {
            ...
            splits.push_back(ChunkUnit{p_text, end_split.is_sentence, get_token_amount(p_text)});
            splits.push_back(ChunkUnit{n_text, end_split.is_sentence, get_token_amount(n_text)});
            end_split = splits.back();
        }

        std::vector<std::string> result;
        for (int idx = static_cast<int>(splits.size()) - 2; idx >= 0; --idx) {
            ChunkUnit start_split = splits[static_cast<size_t>(idx)];
            ...
            if (overlap_len > 0) {
                ...
                end_split = ChunkUnit{overlap_text + end_split.text, end_split.is_sentence,
    ...
    int get_node_metadata_size(const DocNode* node) const {
        return std::max(
            get_token_amount(node->get_metadata_string(MetadataMode::EMBED)),
            get_token_amount(node->get_metadata_string(MetadataMode::LLM)));
    }

    int get_token_amount(const std::string_view& view) const {
