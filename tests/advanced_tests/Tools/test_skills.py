...
class TestSkills(object):
    ...
    def test_run_script_accepts_supported_script_outside_scripts_directory(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            manager = SkillManager(dir=tmp)
            result = manager.run_script('core-script-skill', 'core/check.py', allow_unsafe=True)
    ...
    def test_run_script_rejects_unsupported_file_type(self):
        with tempfile.TemporaryDirectory() as tmp:
            skill_dir = _make_skill(tmp, 'document-skill', 'document-skill')
            reference = os.path.join(skill_dir, 'REFERENCE.md')
            with open(reference, 'w', encoding='utf-8') as f:
                f.write('# Reference\n')

            manager = SkillManager(dir=tmp)
            result = manager.run_script('document-skill', 'REFERENCE.md', allow_unsafe=True)

            assert result['status'] == 'error'
            assert result['error_type'] == 'UnsupportedScriptType'
