...
data = [
    {
        ...
        "embedding": {
            "vec_dense": [0.1, 0.2, 0.3],
            "vec_sparse": {"1563": 0.212890625, "238": 0.1768798828125},
        },
        "type": 1,
        "number": 0,
        "kb_id": "kb1",
    ...
    },
    {
        ...
        "embedding": {
            "vec_dense": [0.3, 0.2, 0.1],
            "vec_sparse": {"12": 0.212890625, "23": 0.1768798828125},
        },
        "type": 1,
        "number": 0,
        "kb_id": "kb3",
...
class TestMapStore(unittest.TestCase):
    ...
    def test_get_segments_by_kb_id(self):
        ...
        self.store1.upsert(self.collections[1], [data[1]])
        res = self.store1.get(
            collection_name=self.collections[0], criteria={RAG_KB_ID: "kb1"}
        )
        self.assertEqual(len(res), 1)
        self.assertEqual(res[0], data[0])
        res = self.store1.get(
            collection_name=self.collections[0], criteria={RAG_KB_ID: "kb3"}
        )
        self.assertEqual(len(res), 1)
        self.assertEqual(res[0], data[2])
        res = self.store1.get(
            collection_name=self.collections[0], criteria={RAG_KB_ID: "kb2"}
        )
        self.assertEqual(len(res), 0)
    ...
    def test_get_segments_by_uid(self):
        ...
        self.store1.upsert(self.collections[1], [data[1]])
        res = self.store1.get(
            collection_name=self.collections[0], criteria={"uid": ["uid1"]}
        )
        self.assertEqual(len(res), 1)
        self.assertEqual(res[0], data[0])
        res = self.store1.get(
            collection_name=self.collections[0], criteria={"uid": ["uid3"]}
        )
        self.assertEqual(len(res), 1)
        self.assertEqual(res[0], data[2])
        res = self.store1.get(
            collection_name=self.collections[0], criteria={"uid": ["uid2"]}
        )
        self.assertEqual(len(res), 0)
    ...
    def test_get_segments_by_doc_id(self):
        ...
        self.store1.upsert(self.collections[1], [data[1]])
        res = self.store1.get(
            collection_name=self.collections[0], criteria={RAG_DOC_ID: ["doc1"]}
        )
        self.assertEqual(len(res), 1)
        self.assertEqual(res[0], data[0])
        res = self.store1.get(
            collection_name=self.collections[0], criteria={RAG_DOC_ID: ["doc2"]}
        )
        self.assertEqual(len(res), 0)
        res = self.store1.get(
            collection_name=self.collections[0], criteria={RAG_DOC_ID: ["doc1", "doc3"]}
        )
        self.assertEqual(len(res), 2)
...
class TestChromadbStore(unittest.TestCase):
    def setUp(self):
        self.data = [
            ...
            },
            {
                ...
                "meta": {},
                "global_meta": {RAG_DOC_ID: "doc2", RAG_KB_ID: "kb2"},
                "embedding": {"vec_dense": [0.3, 0.2, 0.1]},
                "type": 1,
                "number": 0,
                "kb_id": "kb2",
                "excluded_embed_metadata_keys": ["file_size", "file_name", "file_type"],
        ...
        self.store.connect(
            ...
            embed_datatypes=self.embed_datatypes,
    ...
    def test_get_segments_by_kb_id(self):
        ...
        self.assertEqual(len(res), 0)
        res = self.store.get(
            collection_name=self.collections[1], criteria={RAG_KB_ID: "kb2"}
        )
        self.assertEqual(len(res), 1)
        self.assertEqual(res[0].get("uid"), self.data[1].get("uid"))
    ...
    def test_get_segments_by_uid(self):
        ...
        self.assertEqual(len(res), 0)
        res = self.store.get(
            collection_name=self.collections[1], criteria={"uid": ["uid2"]}
        )
        self.assertEqual(len(res), 1)
        self.assertEqual(res[0].get("uid"), self.data[1].get("uid"))
...
class TestMilvusStore(unittest.TestCase):
    def setUp(self):
        ...
        self.embed_datatypes = {
            "vec_dense": DataType.FLOAT_VECTOR,
            "vec_sparse": DataType.SPARSE_FLOAT_VECTOR,
        }
        self.global_metadata_desc = BUILDIN_GLOBAL_META_DESC
        self.index_kwargs = [
            {
    ...
    def test_get_segments_by_kb_and_doc(self):
        ...
        res = self.store.get(
            collection_name=self.collections[0],
            criteria={RAG_KB_ID: "kb1", RAG_DOC_ID: ["doc1"]},
        )
        self.assertEqual(len(res), 1)
        self.assertEqual(res[0].get("uid"), data[0].get("uid"))
    ...
    @pytest.mark.skip(
        reason=(
            "local test for milvus standalone, please set up a milvus standalone server"
            " and set the uri to the server"
        )
    )
    def test_milvus_standalone(self):
        self.store1 = MilvusStore(
            uri=self.uri_standalone, index_kwargs=self.index_kwargs
        )
        self.store1.connect(
            embed_dims=self.embed_dims,
            embed_datatypes=self.embed_datatypes,
...
@pytest.mark.skip(
    reason="To test open search store, please set up a open search server"
)
...
@pytest.mark.skip(
    reason="To test elastic search store, please set up a elastic search server"
)
class TestElasticSearchStore(unittest.TestCase):
    def setUp(self):
        self.collections = ["col_g1", "col_g2"]
        self.uri = ["localhost:9200"]
        self.client_kwargs = {}
...
@pytest.mark.skip(
    reason="To test sensecore store, please set up a sensecore rag-store server"
)
class TestSenseCoreStore(unittest.TestCase):
    def setUp(self):
        ...
        self.store = SenseCoreStore(
            uri=self.uri,
            s3_config=self.s3_config,
            image_url_config=self.image_url_config,
        )
    ...
    def test_upsert(self):
        ...
        res = self.store.get(
            collection_name=self.collections[0],
            criteria={RAG_KB_ID: self.data[0].get("kb_id")},
        )
        self.assertEqual(len(res), 1)
        self.assertEqual(res[0].get("uid"), self.data[0].get("uid"))
        res = self.store.get(
            collection_name=self.collections[1],
            criteria={RAG_KB_ID: self.data[1].get("kb_id")},
        )
        self.assertEqual(len(res), 1)
        self.assertEqual(res[0].get("uid"), self.data[1].get("uid"))
    ...
    def test_delete_segments_by_doc_id(self):
        ...
        self.store.delete(
            self.collections[0],
            criteria={RAG_DOC_ID: ["doc2"], RAG_KB_ID: self.data[0].get("kb_id")},
        )
        res = self.store.get(
            collection_name=self.collections[0],
            criteria={RAG_KB_ID: self.data[0].get("kb_id")},
...
class TestHybridStore(unittest.TestCase):
    def setUp(self):
        ...
        self.vector_store = MilvusStore(
            uri=self.store_dir, index_kwargs=self.index_kwargs
        )
