...
class TestMineruServer(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.server = MineruServerModule(cache_dir=cls.cache_dir, pythonpath=None, port=31769)
        cls.server.start()
        print(cls.server._url)
    ...
    def setup_test_files(self):
                ...
                if not file_path.exists():
                    ...
                    if pdf_name == "pdf2":
                        try:
                            ...
                            print(f"⚠️ 为{pdf_name}添加图片失败: {img_error}")

                    c.save()
                    print(f"✅ 创建PDF文件: {pdf_name}")
                    if pdf_name == "pdf2":
                        print("   📷 包含测试图片和表格")
        ...
        try:
            ...
            if not pptx_file.exists():
                ...
                content.text = "这是一个测试PowerPoint文档\n包含中文内容\n用于测试演示文稿转换功能"
                
                prs.save(str(pptx_file))
                print(f"✅ 创建PowerPoint文档: {pptx_file.name}")
                
        except ImportError:
            print("❌ 缺少python-docx或python-pptx库，无法创建Office文档")
            print("请安装: pip install python-docx python-pptx")
            return False
        except Exception as e:
            print(f"❌ 创建Office文档失败: {str(e)}")
            return False
        
        print("✅ 测试文件设置完成")
        print(f"📄 PDF文件: {len(pdf_contents)} 个")
        print(f"📄 Office文档: 2 个")
        return True
    
    def post_pdf_parse(self, files, backend="pipeline", return_md=True, return_content_list=True, use_cache=False):
    ...
    def test_pdf_parsing(self):
        """测试1: 初始PDF解析（创建缓存基础）"""
        print("\n📋 测试1: 初始PDF解析（创建缓存基础）")
        print("🔄 这一步将解析PDF文件并创建缓存，为后续测试做准备")

        initial_files = [str(self.test_files["pdf1"]), str(self.test_files["pdf2"])]
        print(f"📄 解析PDF文件: {[os.path.basename(f) for f in initial_files]}")
        ...
        print(f"✅ 初始PDF解析成功")
        print(f"📊 处理文件数: {len(result['result'])}")
        ...
        for i, file_result in enumerate(result['result']):
            ...
            if 'md_content' in file_result:
                print(f"   - MD内容: {file_result['md_content']}")
            if 'content_list' in file_result:
                print(f"   - Content List: {file_result['content_list']} ")
    ...
    def test_pdf_parsing_with_upload_files(self):
        ...
        with open(file_path, 'rb') as f:
            upload_files = [('upload_files', (os.path.basename(file_path), f.read(), 'application/pdf'))] 
    ...
    def test_pdf_parsing_with_cache(self):
        ...
        print("\n🔄 测试3: 混合PDF缓存和新文件（依赖测试1和3）")
        print("🔄 这一步将测试部分PDF文件使用缓存，部分PDF文件重新解析")
        
        # 混合文件：第一个有缓存，第三个是新文件
        mixed_files = [str(self.test_files["pdf1"]), str(self.test_files["pdf3"])]
        print(f"📄 混合PDF文件:")
        print(f"   - 有缓存: {os.path.basename(mixed_files[0])}")
        print(f"   - 新文件: {os.path.basename(mixed_files[1])}")
        ...
        for i, file_result in enumerate(result['result']):
            ...
            assert 'md_content' in file_result or 'content_list' in file_result, f"文件{file_name}应包含解析内容"
            
            if 'md_content' in file_result:
                print(f"   - MD内容: {len(file_result['md_content'])} 字符")
            if 'content_list' in file_result:
                print(f"   - Content List: {len(file_result['content_list'])} 项")
            
            # 检查是否使用了缓存
            if i == 0:
                original_result = self.__class__.test_results.get(file_path)
                if original_result:
                    assert file_result.get('md_content') == original_result.get('md_content')
                    assert file_result.get('content_list') == original_result.get('content_list')
    ...
    def test_office_document_parsing(self):
        ...
        office_files = [
            str(self.test_files["docx"]),
            str(self.test_files["pptx"])
        ]
        ...
        for file_path in office_files:
            ...
            print(f"✅ {os.path.basename(file_path)} 解析成功")
    ...
    def test_pdf_reader(self):
        ...
        cache_res = [item['text'] for item in self.__class__.test_results[pdf_path]['content_list']]
        assert set([node._content for node in nodes]) == set(cache_res)
        print([node._content for node in nodes])
    ...
    def test_pdf_reader_with_upload_files(self):
        ...
        # 使用assert验证PDF reader
        assert isinstance(nodes, list)
        cache_res = [item['text'] for item in self.__class__.test_results[pdf_path]['content_list']]
        assert set([node._content for node in nodes]) == set(cache_res)
        print([node._content for node in nodes])
        
        print(f"✅ PDF reader 文件上传模式测试成功")
