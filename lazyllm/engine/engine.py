    ...
    def build(self, node: Node):
        ...
        if node.kind in NodeConstructor.builder_methods:
            ...
            node.func = (
                createf(**node.args)
                if isinstance(node.args, dict)
                and set(node.args.keys()).issubset(
...
def make_http_tool(
    ...
    instance = lazyllm.tools.HttpTool(
        method, url, params, headers, body, timeout, proxies, code_str, vars_for_code
    )
    if doc:
...
class VQA(lazyllm.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.vqa = self._vqa = (
            lazyllm.TrainableModule(base_model).deploy_method(lazyllm.deploy.LMDeploy)
...
def make_shared_llm(
    llm: str, prompt: Optional[str] = None, file_resource_id: Optional[str] = None
):
    llm = Engine().build_node(llm).func
    if file_resource_id:
        assert isinstance(llm, VQA), 'file_resource_id is only supported in VQA'
    return (
        VQA(llm._vqa.share(prompt=prompt), file_resource_id)
        if file_resource_id
        else llm.share(prompt=prompt)
    )
...
def make_stt(base_model: str):
    ...
    return lazyllm.ifs(
        cond, tpath=lazyllm.TrainableModule(base_model), fpath=lazyllm.Identity()
    )
