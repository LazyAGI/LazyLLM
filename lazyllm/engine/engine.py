...
class Engine(object):
    ...
    @overload
    def update(self, gid: str, nodes: List[Dict], edges: List[Dict] = [], resources: List[Dict] = []) -> str: ...
...
class NodeConstructor(object):
    ...
    def build(self, node: Node):
        ...
        node.enable_data_reflow = (
            node.args.pop("_lazyllm_enable_report", False) if isinstance(node.args, dict) else False
        )
        if node.kind in NodeConstructor.builder_methods:
            ...
            node.func = (
                ...
                and set(node.args.keys()).issubset(set(inspect.getfullargspec(createf).args))
                else createf(node.args)
            )
...
def make_graph(
    nodes: List[dict], edges: List[Union[List[str], dict]] = [], resources: List[dict] = [], enable_server=True
):
    engine = Engine()
    ...
    for resource in resources:
        if resource["kind"] in server_resources:
            ...
            server_resources[resource["kind"]] = Node(
                id=resource["id"], kind=resource["kind"], name=resource["name"], args=resource["args"]
            )
...
def make_loop(
    ...
    edges: List[dict] = [],
    resources: List[dict] = [],
    judge_on_full_input: bool = True,
):
    stop_condition = make_code(stop_condition)
    return lazyllm.loop(
        make_graph(nodes, edges, resources, enable_server=False),
...
def make_intention(
    base_model: str, nodes: Dict[str, List[dict]], prompt: str = "", constrain: str = "", attention: str = ""
):
    with IntentClassifier(
        Engine().build_node(base_model).func, prompt=prompt, constrain=constrain, attention=attention
    ) as ic:
        for cond, nodes in nodes.items():
            if isinstance(nodes, list) and len(nodes) > 1:
...
def make_document(
    dataset_path: str, embed: Node = None, create_ui: bool = False, server: bool = False, node_group: List = []
):
...
def make_fc(llm: str, tools: List[str], algorithm: Optional[str] = None):
    f = (
        ...
        else lazyllm.tools.ReWOOAgent
        if algorithm == "ReWOO"
        else lazyllm.tools.ReactAgent
        if algorithm == "React"
...
def make_http_tool(
    ...
    instance = lazyllm.tools.HttpTool(method, url, params, headers, body, timeout, proxies, code_str, vars_for_code)
    if doc:
