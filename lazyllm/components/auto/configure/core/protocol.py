...
CTX_LEN = Rule.from_indexed("CTX_LEN", [128, 512, 1024, 2048], matches=SearchMode.BINARY_CEIL)
BATCH_SIZE = Rule.from_indexed("BATCH_SIZE", [1, 4, 8, 32, 128, 512], matches=SearchMode.BINARY_CEIL)
MAX_TOKEN_NUM = Rule.from_indexed("MAX_TOKEN_NUM",
                                  [1024, 64 * 1024, 256 * 1024, 1024 * 1024], matches=SearchMode.BINARY_CEIL)
LORA_R = Rule.from_indexed("LORA_R", [8, 16, 32], matches=SearchMode.BINARY_CEIL)

# VALUES
FINETUNE_FRAMEWORK = Rule.from_options("FRAMEWORK", ["EASYLLM", "ALPACALORA", "COLLIE"])
