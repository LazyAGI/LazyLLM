...
class OnlineEmbeddingModuleBase(ModuleBase):
    ...
    def is_list_of_str(self, var):
        if not isinstance(var, list):
            return False
        return all(isinstance(x, str) for x in var)

    def _split_data(self, json: Dict, batch_size: int = 2):
        text = json
        for key in self.post_input_key_json:
            text = text.get(key, None)
            if text is None:
                raise "input data is None"
        if not self.is_list_of_str(text):
            raise "input data is invalid"
        if len(text) <= batch_size:
            return [json]
        text_batch = [text[i: i + batch_size] for i in range(0, len(text), batch_size)]
        ret = [copy.deepcopy(json) for _ in range(0, len(text_batch))]
        for i in range(0, len(text_batch)):
            target_json = ret[i]
            for key in self.post_input_key_json:
                target_json = target_json[key]
            target_json = text_batch[i]
        return ret
