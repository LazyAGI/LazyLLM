...
class Document(ModuleBase):
    ...
    registered_node_parser = dict()
    nodes_dict = dict()

    def __init__(self, doc_path, embed, doc_name='Chinese'):
        super().__init__()
        ...
        self.docs = None
        self.nodes_dict = copy.deepcopy(Document.nodes_dict)
    ...
    @classmethod
    def register_node_parser(cls, func):
        cls.registered_node_parser[func.__name__] = func
        return func
    
    @classmethod
    def get_parser(self, parser_name):
        if parser_name in self.registered_node_parser:
            return self.registered_node_parser[parser_name]
        else:
            module = importlib.import_module('llama_index.core.node_parser')
            clazz = getattr(module, parser_name, None)
            if clazz is None:
                raise ImportError(
                    f"Class '{parser_name}' is not registered and cannot be found "
                    "in '{module.__name__}'. Please use 'register_node_parser' to register.")
            return clazz

    def add_parse(self, name, transform, parent=None, **kw):
        assert name not in self.nodes_dict
        self.nodes_dict[name] = {
            'parser': self.get_parser(transform),
            'parser_kw': kw,
            'parent_name': parent,
            'nodes': None,
            'retrievers_algo':{},
    ...
    def forward(self, string):
        if not self.default_retriever:
            self.default_retriever = Retriever(self, algo='defatult',
                                               parser='SentenceDivider', similarity_top_k=3)
    ...
    def _get_node(self, name):
        ...
        if node['nodes'] is not None:
            return node
        if name in self.store.node_record:
...
class Retriever(ModuleBase):
    ...
    def __init__(self, doc, parser, algo='defatult', store='vector',**kw):
        super().__init__()
        self.doc = doc
        self.algo = algo
        self.algo_kw = kw
...
class LazyStore(object):
    ...
    def get_nodes(self, nodes_name, nodes): pass
...
class RedisStore(LazyStore):
    ...
    def get_nodes(self, nodes_name):
        assert nodes_name in self.node_record
    ...
    def index_record(self):
        assert not self.first, "Redis client not initialized."
    ...
    def build_vector_store_context(self, nodes_name):
        assert not self.first, "RedisStore not initialized."
