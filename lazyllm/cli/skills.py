...
def skills(commands):  # noqa: C901
    ...
    parser.add_argument('command', type=str, help='command', choices=['init', 'list', 'info', 'delete', 'add', 'import'])
    ...
    if args.command == 'init':
        roots = manager._parse_dirs(lazyllm.config['skills_dir'])
        home_skills = os.path.join(lazyllm.config['home'], 'skills')
        if home_skills not in roots:
            roots.insert(0, home_skills)
        for path in roots:
            os.makedirs(path, exist_ok=True)
        lazyllm.LOG.success(f'Initialized skills directories: {", ".join(roots)}')
        return
    ...
    if args.command == 'info':
        ...
        if res.get('status') != 'ok':
            ...
            if status == 'missing':
                ...
                lazyllm.LOG.error(f'SKILL.md is too large for "{args.name}". Use --allow-large to override.{extra}')
            else:
                msg = reason or res
    ...
    if args.command == 'import':
        ...
        if installed:
            lazyllm.LOG.info(f'Installed skills: {", ".join(installed)}')
        return
