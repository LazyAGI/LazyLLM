...
Args:
    model (str): 指定要访问的模型 (注意豆包这里需要使用Model ID或者Endpoint ID，请参见 [获取推理接入点](https://www.volcengine.com/docs/82379/1099522))，默认为 ``gpt-3.5-turbo(openai)`` / ``SenseChat-5(sensenova)`` / ``glm-4(glm)`` / ``moonshot-v1-8k(kimi)`` / ``qwen-plus(qwen)`` / ``mistral-7b-instruct-v0.2(doubao)`` 
