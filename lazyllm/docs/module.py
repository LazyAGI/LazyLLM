...
add_chinese_doc('inject_ocr_config', '''\
向当前会话注入请求级 OCR 配置，用于 DynamicPDFReader / OCR Reader 在运行时覆盖默认参数。

注入后配置写入 ``globals.config``，作用域为当前请求会话，常用于多租户或按用户传入不同 OCR 参数。

Args:
    ocr_config (Dict[str, Any], optional): OCR 配置字典，支持字段：
        - ocr_type / ocr_url / ocr_dynamic / mineru_upload_mode
        - mineru_api_key / paddle_api_key
        - ocr_auth: 通用鉴权映射（如 {'mineru': 'token-a', 'paddleocr': 'token-b'}），
          对应写入 ``globals.config['dynamic_ocr_auth']``。

Note:
    ``ocr_auth`` 仅负责“提供 token”，具体注入方式由 reader 的 ``auth_strategy`` 决定。
''')

add_english_doc('inject_ocr_config', '''\
Inject per-request OCR configuration into the current session so DynamicPDFReader / OCR readers
can override default runtime parameters.

Injected values are stored in ``globals.config`` and scoped to the current request session.
This is typically used for multi-tenant or user-specific OCR settings.

Args:
    ocr_config (Dict[str, Any], optional): OCR config dictionary. Supported keys include:
        - ocr_type / ocr_url / ocr_dynamic / mineru_upload_mode
        - mineru_api_key / paddle_api_key
        - ocr_auth: generic auth mapping (e.g. {'mineru': 'token-a', 'paddleocr': 'token-b'}),
          persisted into ``globals.config['dynamic_ocr_auth']``.

Note:
    ``ocr_auth`` only provides tokens. The actual injection format is determined by each reader's
    ``auth_strategy``.
''')

add_example('inject_ocr_config', '''\
>>> import lazyllm
>>> lazyllm.inject_ocr_config({
...     'ocr_type': 'mineru',
...     'ocr_dynamic': True,
...     'ocr_auth': {'mineru': 'sk-user-a'}
... })
''')