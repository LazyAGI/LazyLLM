...
Args:
    ...
    **kwargs: 透传给具体 FS 实现（如 base_url、access_key、secret_key、endpoint_url、region_name 等），也可包含高级 fsspec 选项（asynchronous、use_listings_cache、skip_instance_cache、loop），一般无需设置。

认证与配置:
    配置项 cloudfs_platform（环境变量 CLOUDFS_PLATFORM）可指定默认平台。
    未传 platform 时，按各平台 token 环境变量首次匹配到的平台作为默认平台；token 未传时从该平台对应环境变量读取。
    各平台支持通过环境变量或 config 配置：如 feishu 用 FEISHU_APP_ID/FEISHU_APP_SECRET 或 CLOUDFS_FEISHU_APP_ID/CLOUDFS_FEISHU_APP_SECRET；s3 用 AWS_ACCESS_KEY_ID、AWS_SECRET_ACCESS_KEY、CLOUDFS_S3_ENDPOINT_URL 等；详见各 *FS 文档。
''')
...
Args:
    ...
    **kwargs: Passed to concrete FS (e.g. base_url, access_key, secret_key, app_id, app_secret, endpoint_url, region_name) and may include advanced fsspec options (asynchronous, use_listings_cache, skip_instance_cache, loop), which are rarely needed.
...
Auth and config:
    ...
    Per-platform options can be set via env or config (e.g. feishu: FEISHU_APP_ID/FEISHU_APP_SECRET or CLOUDFS_FEISHU_APP_ID/CLOUDFS_FEISHU_APP_SECRET; s3: AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, CLOUDFS_S3_ENDPOINT_URL). See each *FS doc for details.
''')
_add_fs_example('CloudFS', '''\
>>> from lazyllm.tools.fs import CloudFS
>>> fs = CloudFS(platform='s3', access_key='xxx', secret_key='yyy')
>>> fs.ls('/my-bucket')
>>> fs = CloudFS(platform='feishu', app_id='cli_xxx', app_secret='xxx')
>>> fs.ls('/')
''')
...
环境变量（CloudFS 自动解析）: 持久化方式 FEISHU_APP_ID、FEISHU_APP_SECRET（或 CLOUDFS_FEISHU_APP_ID、CLOUDFS_FEISHU_APP_SECRET）。
...
Env vars (used by CloudFS): Persistent FEISHU_APP_ID, FEISHU_APP_SECRET (or CLOUDFS_FEISHU_APP_ID, CLOUDFS_FEISHU_APP_SECRET).
...
环境变量: 推荐使用 CLOUDFS_GOOGLEDRIVE_CREDENTIALS 或 GOOGLE_DRIVE_CREDENTIALS / GDRIVE_CREDENTIALS 指向 JSON 凭证路径；不再建议直接通过环境变量注入短效 access_token。
...
Env vars: Prefer CLOUDFS_GOOGLEDRIVE_CREDENTIALS or GOOGLE_DRIVE_CREDENTIALS / GDRIVE_CREDENTIALS pointing to the JSON credentials file; passing short-lived access tokens via env is no longer recommended.
...
环境变量: 推荐通过 CLOUDFS_ONEDRIVE_CLIENT_ID / CLOUDFS_ONEDRIVE_CLIENT_SECRET / CLOUDFS_ONEDRIVE_TENANT_ID（或 ONEDRIVE_CLIENT_ID / MSGRAPH_CLIENT_ID 等）配置应用凭据；不再建议直接注入短效 access_token。
...
Env vars: Prefer CLOUDFS_ONEDRIVE_CLIENT_ID / CLOUDFS_ONEDRIVE_CLIENT_SECRET / CLOUDFS_ONEDRIVE_TENANT_ID (or ONEDRIVE_CLIENT_ID / MSGRAPH_CLIENT_ID etc.) for app credentials; passing short-lived access tokens via env (e.g. ONEDRIVE_TOKEN, MSGRAPH_TOKEN) is no longer recommended.
