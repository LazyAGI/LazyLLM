...
飞书云盘文件系统：使用飞书开放平台 Drive API，支持 ls、读写、mkdir、rm、watch/webhook。
...
Feishu (Lark) drive FS: uses Feishu Open Platform Drive API; supports ls, read/write, mkdir, rm, watch/webhook.
...
Confluence 文件系统：基于 Atlassian Confluence REST API，以 Space/Page 为目录与文件，支持 webhook。
...
Confluence FS: Atlassian Confluence REST API; Space/Page as dir/file; supports webhook.
...
Google Drive 文件系统：使用 Drive v3 API，支持 OAuth token 或 Service Account 凭证；支持 watch/webhook。
...
Google Drive FS: Drive v3 API; OAuth token or Service Account credentials; supports watch/webhook.
...
路径格式为 /bucket 或 /bucket/prefix/key。支持 ls、info、读写、mkdir（桶/前缀）、rm、watch（桶事件通知）。
...
Path format: /bucket or /bucket/prefix/key. Supports ls, info, read/write, mkdir (bucket/prefix), rm, watch (bucket events).
