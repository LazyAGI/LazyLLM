...
Args:
    ...
    max_retries (int): 首次工具调用尝试后的最大重试次数。实际最多执行 max_retries + 1 轮工具调用，默认值为5。
...
Args:
    ...
    max_retries (int): 首次工具调用尝试后的最大重试次数。实际最多执行 max_retries + 1 轮工具调用，默认 5。
...
Args:
    ...
    max_retries (int): 首次工具调用尝试后的最大重试次数；实际最多执行 max_retries + 1 轮工具调用。超出后若 `force_summarize=True` 则触发强制总结，否则抛出异常，默认为5
