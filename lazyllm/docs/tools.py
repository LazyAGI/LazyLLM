...
Initialize the MCP client that can be used to connect to an MCP server. Both stdio client and sse client are supported.
...
Args:
    ...
    args (list[str], optional): Arguments list used for starting a local server, if you want to connect to a remote server, this argument is not needed.
    env (dict[str, str], optional): environment variables used in tools, for example some api keys.
''')

add_chinese_doc('mcp.MCPClient', '''\
初始化一个 MCP 客户端，该客户端可用于连接 MCP 服务器。同时支持 stdio 客户端和 sse 客户端。
...
Args:
    ...
    args (list[str], optional): 用于启动本地服务器的参数列表；如果要连接远程服务器，则无需此参数。
    env (dict[str, str], optional): 工具中使用的环境变量，例如一些 API 密钥。
''')
...
... )
>>> tool_list = asyncio.run(file_client.list_tools())
>>> allow_paths = asyncio.run(file_client.call_tool(tool_name='list_allowed_directories', arguments={}))
''')
...
add_english_doc('mcp.McpToolAdaptor', '''\
Converts MCP tools fetched from the MCP client to functions which can be used by the LazyLLM agent.

Args:
    client (mcp.ClientSession): mcp client which connects to the MCP server.
    allowed_tools (list[str], optional): The list of tools that can be used by the LazyLLM agent. If this argument is not provided, all tools will be used.
''')

add_chinese_doc('mcp.McpToolAdaptor', '''\
将从 MCP 客户端获取的工具转换为可供 LazyLLM 智能体使用的函数。

Args:
    client (mcp.ClientSession): 连接到 MCP 服务器的 MCP 客户端。
    allowed_tools (list[str], optional): 可供 LazyLLM 智能体使用的工具列表。如果未提供此参数，则将使用所有工具。
''')
