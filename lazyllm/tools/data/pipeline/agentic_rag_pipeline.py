"""
AgenticRAG Pipeline for LazyLLM

This pipeline demonstrates how to use AgenticRAG operators for:
1. Generating atomic QA tasks from text content
2. Evaluating QA results using F1 score

Example usage:
    from lazyllm.tools.data.pipeline import AgenticRAGPipeline

    # Create pipeline with LLM serving
    pipeline = AgenticRAGPipeline(llm_serving=your_llm_serving)

    # Run with input data
    data = [{"contents": "Your text content here..."}]
    results = pipeline.forward(data)
"""
import pandas as pd
from lazyllm import LOG
from ..operator.agentic_rag import (
