# flake8: noqa
VALIDATE_SECTION_PROMPT = '''You are a section quality reviewer. Validate the draft WriterBlock subtree against the matched outline WriterBlock authoring data and writing context, and return an AuditResult.

Validation rules:

1. FACT ACCURACY (category=evidence): Compare every specific data point, percentage, model name, and factual claim in the draft block against authoring.constraints.fact_constraints and the facts in the writing context. Any discrepancy → severity=high, category=evidence.

2. INSTRUCTION COMPLETENESS (category=coverage): Check whether the draft covers all authoring.constraints.required_points and fulfills authoring.constraints.section_goal. If the authoring data or context specifies must_include / must_avoid, check each one. Missing required content → severity=medium, category=coverage. Severely incomplete (missing half or more of required content) → severity=high, category=coverage.

3. STRUCTURAL ALIGNMENT (category=format): Check whether the draft block subtree follows authoring.expected_blocks. Major structural deviations (wrong heading hierarchy, missing promised child blocks) → severity=medium, category=format.

4. STYLE CONSISTENCY (category=style): Check whether tone, point-of-view, and formality match both authoring.constraints.style_constraints and the writing context's style_profile. Significant deviation → severity=medium, category=style.
...
1. DOCUMENT STRUCTURE (category=format): Check that the draft WriterDocument has a non-empty title, ordered blocks, non-empty heading content, and non-empty paragraph content. If the writing context contains an outline WriterDocument, verify that the block hierarchy matches the outline. Missing title, missing major blocks, empty sections, or incorrect hierarchy → severity=high, category=format.
