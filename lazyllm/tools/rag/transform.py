...
from lazyllm.prompts import LLMTransformParserPrompts, create_llm_transform_prompts
...
class LLMParser(NodeTransform):
    ...
    def __init__(
        ...
        super(__class__, self).__init__(num_workers=num_workers)
        language_full = {"en": "English", "zh": "Chinese"}
        assert language in language_full, f"Not supported language {language}"
        assert task_type in ["summary", "keywords", "qa", "qa_img"], f"Not supported task_type {task_type}"
        self._task_type = task_type
        self._prompts = prompts or create_llm_transform_prompts(language=language_full[language])
