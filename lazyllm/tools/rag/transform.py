...
class LLMTransformParserPrompts:
    ...
    summary_en: str = textwrap.dedent("""
    ...
    summary_zh: str = textwrap.dedent("""
        ## 角色：文本摘要
    ...
    keywords_en: str = textwrap.dedent("""
    ...
    keywords_zh: str = textwrap.dedent("""
        ## 角色：关键词提取引擎
    ...
    qa_en: str = textwrap.dedent("""
        ...
        You should not have any unnecessary output. Lets begin:
    """).format(num_pairs_min=3, num_pairs_max=5) # noqa E501

    qa_zh: str = textwrap.dedent("""
        ## 角色：问答生成助手
        你是一个问答生成助手。您的任务是根据上下文生成 {num_pairs_min} 到 {num_pairs_max} 个问答对。

        ## 约束条件:
        - 仅回复请求的输出内容：抽取问答对。
        - 不要添加额外字段、解释或翻译。
        - 每个问题必须能够直接从文本中找到具体答案。
        - 答案应简洁准确。
        - 不得包含上下文未支持的信息。
        - 优先选择事实性强、细节丰富的问题，而非模糊问题。
        - 问题必须自成一体：避免使用模糊指代，如“该文本”、“此段落”、“这里”或“上文”。每个问题应独立成立，无需依赖外部上下文。
...
class LLMParser(NodeTransform):
    ...
    def __init__(
        self, llm: TrainableModule, language: str, task_type: str,
        ...
        task_prompt = getattr(self._prompts, f"{self._task_type}_{language}")
        if self._task_type == "qa_img":
            prompts = dict(system=task_prompt, user="{input}")
        else:
            prompts = dict(system=task_prompt, user="#input:\n{input}\n#output:\n")
        self._llm = llm.share(prompt=AlpacaPrompter(prompts), stream=False, format=self._format)
        self._task_type = task_type

    @property
    def prompts(self) -> LLMTransformParserPrompts:
        """Getter property for configuration prompts"""
