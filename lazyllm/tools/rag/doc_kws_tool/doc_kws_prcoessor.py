...
class DocKwDesc(TypedDict):
    key: str
    desc: str
    type: str


def validate_kw_desc(given_dict: dict, typed_dict_schema: type[dict]) -> Tuple[bool, str]:
...
class DocTypeDetector:
    ...
    def __init__(self, llm, maximum_doc_num=3):
        self._llm_type_detector = llm.share()
    ...
    def get_doc_type(self, doc_path: str) -> str:
        query = self.gen_detection_query(doc_path)
        response = self._llm_type_detector(query)
        doc_type = self._extract_doc_type_from_response(response)
        return doc_type


class DocKWSGenerator:
    ...
    def _extract_kws_desc_from_response(self, str_response: str) -> tuple[bool, str]:
        # Remove the triple backticks if present
        matches = self._pattern.findall(str_response)
        empty_list = []
        if matches:
    ...
    def gen_kws_template(self, doc_type: str, doc_paths: list[str]) -> List[DocKwDesc]:
        RANDOM_SEED = 1331
        ...
        for kw_desc in kws_desc:
            ...
            if not is_success:
                lazyllm.LOG.warning(f"Please Try Again! Invalid kws dict: {kw_desc}, error_msg: {msg}")
                return []
        return kws_desc


class DocKWSExtractor:
    ...
    def _gen_extraction_query(self, doc_path: str, kws_desc: List[DocKwDesc], extra_desc: str) -> str:
        root_nodes = self._reader.load_data([doc_path], None)
        doc_content = ""
        for root_node in root_nodes:
            doc_content += root_node.text + "\n"
        ...
        query = DOC_KWS_PROMPTS["kws_extraction"].format(
            kws_desc=json.dumps(kws_desc), extra_desc=extra_desc, doc_content=doc_content
        )
        return query

    def _extract_kws_value_from_response(self, str_response: str, kws_desc: List[DocKwDesc]) -> dict:
        # Remove the triple backticks if present
        matches = self._pattern.findall(str_response)
        empty_dict = {}
        if matches:
