...
from typing import Callable, Optional, Dict, Union, List, TypeVar
import lazyllm
...
class ABCModuleMeta(type(ABC), type(ModuleBase)):
    pass
...
def require_kwargs(*required_args):
    def decorator(method):
        def wrapper(self, **kwargs):
            missing = set(required_args) - set(kwargs)
            if missing:
...
class BaseDocManager(ABC, ModuleBase, metaclass=ABCModuleMeta):
    def __init__(
        self,
        dataset_path: str,
        ...
        **kwargs,
        ...
        # Initialize kbs in child classes
        self._kbs = None
        self._initialize_required_member(**kwargs)
        assert self._kbs is not None

    @abstractmethod
    def _initialize_required_member(self, **kwargs):
        pass
...
class IDocument(ABC):
    ...
    @abstractmethod
    def __init__(self):
        self._manager: BaseDocManager = None
        self._curr_group: str = ""
...
class Document(IDocument, ModuleBase, metaclass=ABCModuleMeta):
    class _Manager(BaseDocManager):
        ...
        @require_kwargs("embed", "server", "store_conf", "doc_fields")
        def _initialize_required_member(self, **kwargs):
            self._embed = self._get_embeds(kwargs["embed"])
            self._kbs = CallableDict(
                {
                    DocListManager.DEFAULT_GROUP_NAME: DocImpl(
                        embed=self._embed,
                        dlm=self._dlm,
                        global_metadata_desc=kwargs["doc_fields"],
                        store_conf=kwargs["store_conf"],
                    )
                }
            )
            if kwargs["server"]:
                self._kbs = ServerModule(self._kbs)
            self._global_metadata_desc = kwargs["doc_fields"]
        ...
        def add_kb_group(
            ...
            if isinstance(self._kbs, ServerModule):
                self._kbs._impl._m[name] = DocImpl(
                    dlm=self._dlm,
                    embed=embed,
                    kb_group_name=name,
    ...
    def __init__(
        ...
        if isinstance(manager, Document._Manager):
            ...
            if dataset_path != manager._dataset_path and dataset_path != manager._origin_path:
                raise RuntimeError(
                    f"Document path mismatch, expected `{manager._dataset_path}`" f"while received `{dataset_path}`"
                )
            manager.add_kb_group(name=name, doc_fields=doc_fields, store_conf=store_conf, embed=embed)
            self._manager = manager
            self._curr_group = name
        else:
            self._manager = Document._Manager(
                dataset_path, embed, create_ui or manager, server, name, launcher, store_conf, doc_fields
            )
    ...
    def create_kb_group(
        self, name: str, doc_fields: Optional[Dict[str, DocField]] = None, store_conf: Optional[Dict] = None
    ) -> "Document":
    ...
    @DynamicDescriptor
    def add_reader(self, pattern: str, func: Optional[Callable] = None):
        return super().add_reader(pattern, func)
    ...
    def create_node_group(
        ...
        **kwargs,
        ...
        if isinstance(self, type):
            DocImpl.create_global_node_group(
                name, transform=transform, parent=parent, trans_node=trans_node, num_workers=num_workers, **kwargs
            )
        else:
            self._impl.create_node_group(
                name, transform=transform, parent=parent, trans_node=trans_node, num_workers=num_workers, **kwargs
            )
...
class GraphDocument(IDocument, ModuleBase, metaclass=ABCModuleMeta):
    class _Manager(BaseDocManager):
        ...
        def __init__(
            ...
            assert isinstance(embed, ModuleBase)
            assert isinstance(llm, ModuleBase)
            super().__init__(
                dataset_path,
                manager,
                name,
        ...
        @require_kwargs("embed", "llm", "store_conf", "reader_conf")
        def _initialize_required_member(self, **kwargs):
            self._embed = kwargs["embed"]
            self._llm = kwargs["llm"]
            
            self._submodules.append(self._embed)
            self._submodules.append(self._llm)
            self._kbs = CallableDict(
                {
                    DocListManager.DEFAULT_GROUP_NAME: GraphDocImpl(
                        llm=self._llm,
                        embed=self._embed,
                        group_name=DocListManager.DEFAULT_GROUP_NAME,
                        reader_conf=kwargs["reader_conf"],
                        store_conf=kwargs["store_conf"],
    ...
    def __init__(
        ...
        self._manager = GraphDocument._Manager(
            dataset_path, llm, embed, manager, server, name, launcher, store_conf, reader_conf
        )
        self._curr_group = DocListManager.DEFAULT_GROUP_NAME
