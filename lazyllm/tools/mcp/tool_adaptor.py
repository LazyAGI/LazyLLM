...
def generate_lazyllm_tool(client, mcp_tool) -> Callable:
    ...
    identity = json.dumps([server_id, tool_name], ensure_ascii=False) if server_id else ''
    return fc_register(tool_source='mcp', tool_origin=server_id,
                       tool_identity=identity)(dynamic_lazyllm_func)
