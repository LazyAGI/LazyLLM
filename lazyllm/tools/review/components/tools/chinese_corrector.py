...
class ChineseCorrector:
    def __init__(self, llm: Optional[Union[TrainableModule, OnlineModule]] = None,
        ...
        if llm:
            ...
            # base_llm = configs.get_llm_model(source=source, model=model, system_prompt=system_prompt)
...
if __name__ == '__main__':
    ...
    for s in examples:
        print(' -', s)
    # llm = OnlineModule(model='deepseek-chat', source='deepseek')
    corrector = ChineseCorrector(model='deepseek-chat', source='deepseek')
