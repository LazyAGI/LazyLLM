...
from .protocol import Message, ToolCall, ROLE, ASSISTANT, TOOL
from .utils import trans_chat_module_request, trans_chat_module_response

class FuncCallModule(lazyllm.ModuleBase):
    def __init__(self, 
        ...
        with lazyllm.pipeline() as fc_ppl:
            fc_ppl.request = self._prepare_fc_ppl_request
            fc_ppl.llm_ppl = lazyllm.pipeline(trans_chat_module_request, self._call_llm, trans_chat_module_response)
            ...
            fc_ppl.merge_message = self._merge_all_message | lazyllm.bind(fc_ppl.input["messages"], 
                                                                    fc_ppl.llm_ppl,
                                                                    lazyllm._0,)
            fc_ppl.recall_request = self._prepare_fc_ppl_request | lazyllm.bind(tools=fc_ppl.input["tools"], kwargs=fc_ppl.llm_ppl.input["kwargs"])

        self.fc_loop = lazyllm.loop(fc_ppl, stop_condition=lambda x:x["messages"][-1][ROLE]==ASSISTANT)

    def forward(self, messages:List[Message], tools:List[str]=None, **kwargs):
        return self.fc_loop(messages=messages, tools=tools, **kwargs)
    ...
    def _prepare_fc_ppl_request(self, messages:List[Message], tools:List[str], **kwargs):
        kwargs.update(kwargs.pop("kwargs", {}))
        if self._tool_manager:
            if not tools: tools = self._tool_manager.all_tools
