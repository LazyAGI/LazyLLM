...
- In this mode `Document` does not create a `DocServer` or UI, and `dataset_path` is not used for local path monitoring.
- `store_conf` is required and must not be a pure map store.
