...
#   1. Specify the model name (e.g. 'Qwen3-32B'):
#           the model will be automatically downloaded from the Internet;
#   2. Specify the model name (e.g. 'Qwen3-32B') ​​+ set
#      the environment variable `export LAZYLLM_MODEL_PATH="/path/to/modelzoo"`:
#           the model will be found in `path/to/modelazoo/Qwen3-32B/`
#   3. Directly pass the absolute path to TrainableModule:
#           `path/to/modelazoo/Qwen3-32B`

chat = lazyllm.TrainableModule('Qwen3-32B').deploy_method(lazyllm.deploy.vllm)
